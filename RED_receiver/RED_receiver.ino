...
unsigned long bTimeout = 0;
u16 nHoldingTime = 10000;              // 릴레이 동작 유지시간
u16 bHoldingTime = 4000;              // 20km이상 60km이하 신호 유지 시간
...
uint8_t bDetectNow = false;           // 추돌주의 플래그
uint8_t nState = 0;
  ...
  if(Serial.available())
  {
    ...
    if(c == '\n'){
      if(strcmp("$SPDOP", sRecv.substring(0, 6).c_str()) == 0) {
        ...
        if(command == "1")      //레이더에서 차량을 감지 했을경우에만 릴레이 동작
        {      
            if(rSpeed > 5 && rSpeed < 20)
            {
              if(bDetectNow == false)
              {
              rFlag = true;
